<template>
  <div class="container mx-auto px-4 py-8 max-w-6xl">
    ...
    <h1 class="text-3xl font-bold text-gray-800 mb-4">Git SSH Keys Management</h1>
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
      <h2 class="text-lg font-bold text-blue-900 mb-3">How to add Deploy Keys</h2>
    ...
    <div v-else-if="repos.length === 0" class="text-center py-12 bg-white rounded-lg shadow-md">
      <p class="text-gray-600 mb-4">No Git repositories found in your projects</p>
      <router-link to="/projects" class="text-blue-600 hover:text-blue-700 font-medium">
    ...
    <div v-else class="bg-white rounded-lg shadow-md overflow-hidden">
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="repo in repos" :key="repo.gitRepo" class="hover:bg-gray-50 transition">
            <td class="px-6 py-4">
              <div class="text-sm text-gray-900 font-mono">{{ repo.gitRepo }}</div>
            </td>
            <td class="px-6 py-4">
    ...
    <div v-if="selectedRepo" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50"
      ...
      <div class="bg-white rounded-lg p-6 max-w-3xl w-full max-h-[90vh] overflow-y-auto" @click.stop>
        <div class="flex justify-between items-start mb-4">
          <div class="flex-1">
            <h3 class="text-xl font-bold text-gray-800">SSH Deploy Key</h3>
            <p class="text-xs text-gray-500 mt-2 font-mono">{{ selectedRepo.gitRepo }}</p>
            <div class="mt-3 text-sm text-gray-700">
              <p class="font-medium text-gray-600 mb-1">Used by:</p>
          ...
          <button @click="selectedRepo = null" class="text-gray-400 hover:text-gray-600 text-2xl leading-none ml-4">
            ×
          </button>
        </div>
        ...
        <div class="mb-4">
          ...
          <div class="relative">
            <textarea :value="selectedRepo.publicKey" readonly rows="8"
              class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-50 font-mono text-xs"></textarea>
            <button @click="copyKey(selectedRepo.publicKey)"
              class="absolute top-2 right-2 bg-gray-200 hover:bg-gray-300 text-gray-700 px-3 py-1 rounded text-sm transition">
              {{ copied ? 'Copied!' : 'Copy' }}
            </button>
          </div>
        </div>
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h4 class="font-medium text-blue-900 mb-2">How to add this Deploy Key:</h4>
    ...
    <div v-if="confirmRegenerateRepo"
      ...
      <div class="bg-white rounded-lg p-6 max-w-md w-full" @click.stop>
        <h3 class="text-xl font-bold text-gray-800 mb-4">Regenerate SSH Key?</h3>
        <p class="text-gray-600 mb-2">
          This will generate a new SSH key for:
        </p>
        <p class="text-sm font-mono text-gray-800 bg-gray-100 p-2 rounded mb-4">
